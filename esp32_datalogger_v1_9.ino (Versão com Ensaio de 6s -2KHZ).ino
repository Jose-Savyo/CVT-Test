 ...
 *
 * Descrição: ... (o resto do seu cabeçalho) ...
 *
 * =====================================================================================
 *
 * --- NOTA DE ARQUITETURA E COMPROMISSO DE PROJETO (V1.9) ---
 *
 * 1. O PROBLEMA (LIMITE DE MEMÓRIA SRAM):
 * O objetivo inicial era um ensaio de 20 segundos a 3kHz, o que exigiria um
 * buffer contínuo de 360KB (60.000 pontos * 6 bytes).
 * Testes práticos de alocação de memória (Versões V1.1 a V1.7) falharam,
 * mesmo com otimizações (como alocar antes do Wi-Fi).
 *
 * Conclusão do Teste: A ESP32-WROOM (placa padrão, sem PSRAM) não possui
 * 360KB, 180KB, ou mesmo 90KB de SRAM *contígua* disponível após o
 * sistema operacional (FreeRTOS) ser carregado. A memória está
 * fragmentada.
 *
 * 2. A SOLUÇÃO ATUAL (V1.9 - O COMPROMISSO):
 * O maior buffer contíguo que conseguimos alocar de forma estável foi de
 * 72KB (aprox. 12.000 pontos de 6 bytes).
 *
 * Sendo o buffer máximo de 12.000 PONTOS, fomos forçados a um "trade-off"
 * entre Frequência de Coleta e Tempo de Ensaio.
 * (Frequência * Tempo = 12.000)
 *
 * Esta versão V1.9 implementa a decisão de reduzir a frequência de coleta
 * para 2.000 Hz (2kHz) a fim de estender o tempo de ensaio.
 *
 * CÁLCULO ATUAL: 2.000 Hz * 6 Segundos = 12.000 Pontos (Buffer de 72KB)
 *
 * ALERTA DE RISCO: Os sensores têm uma frequência de resposta de 1kHz.
 * Coletar a 2kHz nos coloca *exatamente* no limite mínimo do Teorema de
 * Nyquist. Isso é aceitável para testes, mas arriscado para produção,
 * pois pode levar a uma baixa precisão na detecção do tempo das bordas do sinal
 * ou, no pior caso, a "aliasing" (dados falsos).
 *
 * 3. AS FUTURAS SOLUÇÕES ROBUSTAS:
 * Esta versão V1.9 é uma solução de compromisso funcional. Para atingir
 * os objetivos originais de ensaios mais longos e com maior fidelidade,
 * as soluções corretas são:
 *
 * A) SOLUÇÃO DE HARDWARE (Para Ensaio Único e Longo):
 * Substituir a placa atual por uma **ESP32-WROVER** (ou qualquer
 * modelo com **PSRAM** externa). A PSRAM (4MB ou 8MB) eliminaria
 * completamente o gargalo de memória, permitindo o buffer
 * original de 360KB (ou até maior).
 *
 * B) SOLUÇÃO DE SOFTWARE (Para Ensaio Contínuo):
 * Mudar a arquitetura deste firmware de "buffer único" para um
 * "streaming de buffer duplo (ping-pong)". Usaríamos dois buffers
 * pequenos e seguros (ex: 2x 30KB). O Núcleo 1 encheria um buffer
 * enquanto o Núcleo 0 enviava o outro pela Wi-Fi. Isso permitiria
 * um tempo de ensaio ILIMITADO.
 *
 * =====================================================================================
 */
