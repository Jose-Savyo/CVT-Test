 ...
 * =====================================================================================
 *
 * Filename:  esp32_datalogger_v2_5.ino (Leitura Dinâmica do Clock)
 *
 * Descrição:  Firmware V2.5 para Datalogger ESP32.
 * - Corrige erro de compilação da V2.4 ('soc/clk_freq_defs.h' não encontrado).
 *
 * - *** CORREÇÃO CRÍTICA (V2.5) ***
 * - Remove o include "soc/clk_freq_defs.h".
 * - Em vez de usar a constante 'APB_CLK_FREQ', obtém a frequência do clock APB
 * dinamicamente usando a função 'rtc_clk_apb_freq_get()'. Isso garante
 * maior compatibilidade entre diferentes versões do ESP-IDF / Arduino Core.
 *
 * =====================================================================================
 */
...
// #include "soc/clk_freq_defs.h" // <<<--- REMOVIDO

// <<<--- ADICIONADO: Header necessário para rtc_clk_apb_freq_get() ---<<<
#include "esp_clk.h" // Ou poderia ser #include "soc/rtc.h" dependendo da versão
...
// <<<--- CORRIGIDO: Usa função rtc_clk_apb_freq_get() ---<<<
// A frequência retornada por rtc_clk_apb_freq_get() é geralmente 80000000 (80MHz)
#define TIMER_SCALE           (rtc_clk_apb_freq_get() / TIMER_DIVIDER) // Frequência do timer = 1MHz
#define TIMER_INTERVALO_US    INTERVALO_COLETA_US
#define TIMER_ALARME_VALOR    (TIMER_INTERVALO_US * (TIMER_SCALE / 1000000ULL)) // Alarme = 200 ticks
...
// (Sem mudanças em relação à V2.4)
// =====================================================================================
...
// (Sem mudanças lógicas, mas agora usa TIMER_SCALE calculado dinamicamente)
// =====================================================================================
void timer_init_setup() {
  // <<<--- Adicionado: Imprime a frequência APB detectada ---<<<
  Serial.print("Core 1: Frequência APB detectada: ");
  Serial.print(rtc_clk_apb_freq_get() / 1000000); // Imprime em MHz
  Serial.println(" MHz");
...
// (Sem mudanças)
// =====================================================================================
void taskColetaCore0(void* pvParameters) { /* ... código idêntico ao V2.4 ... */ }

// =====================================================================================
// --- 8. FUNÇÕES DE GERENCIAMENTO (NÚCLEO 1 - loop) ---
// (Sem mudanças)
// =====================================================================================
void gerenciarAguardandoConexao() { /* ... código idêntico ao V2.4 ... */ }
void gerenciarStreaming() { /* ... código idêntico ao V2.4 ... */ }
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("--- Datalogger ESP32 V2.5 (Dynamic Clock) ---");
  ...
  if (g_bufferA == NULL || g_bufferB == NULL) { /* ... código de erro idêntico V2.4 ... */ }
  ...
  // Configura Timer <<<--- Chamada da função atualizada
  timer_init_setup();
...
// --- Funções gerenciarEstado... e taskColetaCore0 (completas) ---
// Cole o código completo dessas funções da V2.4 aqui
