...
# Tamanho de CADA buffer enviado pela ESP32
...
def receber_stream():
    ...
    print(f"--- Cliente Datalogger Python (Receptor de Stream V2.0) ---")
    ...
    try:
        ...
        sock.settimeout(10.0) # Timeout para a conexão inicial
        sock.connect((ESP32_HOST, ESP_PORT))
        sock.settimeout(None) # Remove o timeout para o recebimento do stream
        print("Conectado! Aguardando stream de dados...")

        # 2. Abrir o arquivo binário para escrita (append binary 'ab')
        with open(ARQUIVO_SAIDA, 'wb') as f_bin: # Usar 'wb' para sobrescrever ou 'ab' para adicionar
            ...
            while True:
                ...
                while bytes_faltando_neste_chunk > 0:
                    try:
                        ...
                        raise # Re-levanta a exceção para sair
                ...
                if len(data_chunk) == TAMANHO_CHUNK_BYTES:
                    ...
                    # Isso não deveria acontecer se o firmware estiver correto
