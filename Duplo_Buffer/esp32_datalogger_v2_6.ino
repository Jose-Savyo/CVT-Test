 ...
 * =====================================================================================
 *
 * Filename:  esp32_datalogger_v2_6.ino (Alternative Clock Header)
 *
 * Descrição:  Firmware V2.6 para Datalogger ESP32.
 * - Corrige erro de compilação da V2.5 ('esp_clk.h' não encontrado).
 *
 * - *** CORREÇÃO CRÍTICA (V2.6) ***
 * - Substitui o include "esp_clk.h" por "soc/rtc.h", que é a
 * localização alternativa para a função 'rtc_clk_apb_freq_get()' em
 * algumas versões do ESP-IDF / Arduino Core.
 *
 * =====================================================================================
 */
...
// <<<--- CORREÇÃO: Tenta o header alternativo ---<<<
#include "soc/rtc.h" // <<<--- MUDANÇA AQUI
...
// Usa função rtc_clk_apb_freq_get() que deve estar em soc/rtc.h
...
// =====================================================================================
void IRAM_ATTR timer_isr(void *para) { /* ... código idêntico ao V2.5 ... */ }

// =====================================================================================
// --- 6. FUNÇÃO DE CONFIGURAÇÃO DO TIMER (Chamada no Setup) ---
// (Sem mudanças em relação à V2.5)
// =====================================================================================
void timer_init_setup() { /* ... código idêntico ao V2.5 ... */ }

// =====================================================================================
// --- 7. TAREFA DO NÚCLEO 0 (COLETA DE DADOS - Acordada pelo Timer) ---
// (Sem mudanças)
// =====================================================================================
void taskColetaCore0(void* pvParameters) { /* ... código idêntico ao V2.5 ... */ }

// =====================================================================================
// --- 8. FUNÇÕES DE GERENCIAMENTO (NÚCLEO 1 - loop) ---
// (Sem mudanças)
// =====================================================================================
void gerenciarAguardandoConexao() { /* ... código idêntico ao V2.5 ... */ }
void gerenciarStreaming() { /* ... código idêntico ao V2.5 ... */ }
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("--- Datalogger ESP32 V2.6 (RTC Clock Header) ---"); // <-- Versão atualizada
  ...
  if (g_bufferA == NULL || g_bufferB == NULL) { /* ... código de erro ... */ }
...
// =====================================================================================
void loop() { /* ... código idêntico ao V2.5 ... */ }


// --- Funções Completas (Copiar/Colar das versões anteriores) ---
...
void loop() {
  switch (g_estadoAtual) {
    case ESTADO_AGUARDANDO_CONEXAO: gerenciarAguardandoConexao(); break;
    case ESTADO_STREAMING: gerenciarStreaming(); break;
  }
}

// Código de erro para falha na alocação (no setup)
